...
export default function RemindersScreen() {
  ...
  const requestNotificationPermissions = async () => {
    ...
    if (finalStatus !== 'granted') {
      Alert.alert(
        ...
        'Enable notifications in settings to receive expense reminders.'
      );
    }
  };
  ...
  const scheduleNotification = async (reminder: Reminder) => {
    ...
    // Schedule notification for one day before
    ...
    if (notificationDate > new Date()) {
      const identifier = await Notifications.scheduleNotificationAsync({
        content: {
          ...
          body: reminder.message || `Don't forget: ${reminder.title} is due tomorrow!`,
          sound: true,
        },
        trigger: {
  ...
  const handleSubmit = async () => {
    ...
    const newReminder = { ...reminderData, id: '' } as Reminder;
    ...
    Alert.alert(
      'Success',
      `Reminder ${editingReminder ? 'updated' : 'created'}! You'll be notified one day before.`
    );
  };
      ...
      <View style={[styles.reminderItem, { backgroundColor: colors.card, opacity: isPast ? 0.6 : 1 }]}>
        <TouchableOpacity style={styles.reminderContent} onPress={() => handleEdit(item)}>
          ...
          <View style={styles.reminderDetails}>
            ...
            <Text style={[styles.reminderDate, { color: colors.textSecondary }]}>
              {format(reminderDate, 'dd MMM yyyy')} at {item.time}
            </Text>
            {item.message && (
              <Text style={[styles.reminderMessage, { color: colors.textSecondary }]} numberOfLines={1}>
                {item.message}
              </Text>
            )}
            {item.is_recurring && (
              <View style={styles.recurringBadge}>
                ...
                <Text style={[styles.recurringText, { color: colors.primary }]}>Monthly</Text>
              </View>
            )}
          </View>
        </TouchableOpacity>
        <View style={styles.reminderActions}>
          <Switch
            ...
            thumbColor={item.is_enabled ? colors.primary : colors.textSecondary}
          />
          <TouchableOpacity 
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptySubtitle, { color: colors.textSecondary }]}>
              Add reminders for upcoming expenses
            </Text>
          </View>
        }
      />
            ...
            <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.inputGroup}>
                  ...
                  <TextInput
                    style={[styles.input, styles.textArea, { color: colors.text, backgroundColor: colors.inputBg }]}
                    placeholder="e.g., Pay rent to landlord"
                    placeholderTextColor={colors.textSecondary}
                    value={message}
                    ...
                    <View style={styles.timeGrid}>
                      {timeOptions.map((time) => (
                        <TouchableOpacity
                          ...
                          style={[
                            styles.timeButton,
                            { backgroundColor: selectedTime === time ? colors.primary : colors.inputBg },
                          ]}
                          onPress={() => {
                            Haptics.selectionAsync();
                ...
                <View style={[styles.toggleRow, { backgroundColor: colors.inputBg }]}>
                  <View>
                    ...
                    <Text style={[styles.toggleSubtext, { color: colors.textSecondary }]}>
                      Repeat this reminder every month
                    </Text>
                  </View>
                  <Switch
                    value={isRecurring}
                ...
                <View style={[styles.infoBox, { backgroundColor: colors.primary + '10' }]}>
                  ...
                  <Text style={[styles.infoText, { color: colors.primary }]}>
                    You'll receive a notification one day before the due date at {selectedTime}
                  </Text>
                </View>
                ...
                >
                  <Text style={styles.submitButtonText}>
                    {isSubmitting ? 'Saving...' : editingReminder ? 'Update Reminder' : 'Create Reminder'}
                  </Text>
                </TouchableOpacity>
              </ScrollView>
            </View>
